...
def get_master_df():
    filename = "master_fact_good_date.csv"
    df_master = pd.read_csv(filename, sep=';', names=['datetime', 'user', 'message'])

    log_topic = df_master
    # logging.debug(type(log_topic))
    # logging.debug(len(log_topic))
    # logging.debug(log_topic.columns)
    # logging.debug(log_topic.datetime.min())
...
def create_datetime_type(df):

    df['datetime'] = pd.to_datetime(df['datetime']
                                    , format='%d-%m-%y %H:%M'
                                    )

    logging.debug(df.datetime.min())
    logging.debug(df.datetime.max())
    return df
...
master_df = create_datetime_type(master_df)
...
# df_datetime = create_datetime_type(df_clean)
# df_sorted = sort_datetime(df_datetime)
# change_to_master_dir()
# new_master_df_to_csv(df_sorted)
